...
class MyMultiAGVPlanner(MultiAGVPlanner):
    ...
    def build_adjacency_list(self):
        ...
        for node in self.graph_data.get("nodes", []):
            ...
            nid = node["id"]
            if t == "LOAD":
                self.loads.append(nid)
            elif t == "UNLOAD":
                self.unloads.append(nid)
...
class Controller:
    def __init__(self, broker_url, pub_topic_A, sub_topic_A, pub_topic_B, sub_topic_B):
        ...
        self.start0, self.dest0 = None, None
        self.start1, self.dest1 = None, None
    ...
    def find_path(self, controller, dest0, start1, dest1):
    ...
    def find_point_pairs(self):
        ...
        if(controller.car_plan.start_nodes[0]< controller.car_plan.start_nodes[1]): 
            self.start0, self.start1 = controller.car_plan.start_nodes[0],controller.car_plan.start_nodes[1]
        else: 
            self.start0, self.start1 = controller.car_plan.start_nodes[1],controller.car_plan.start_nodes[0]

        if(controller.car_plan.destination_nodes[0]< controller.car_plan.destination_nodes[1]): 
            self.dest0, self.dest1 = controller.car_plan.destination_nodes[0], controller.car_plan.destination_nodes[1]
        else: 
            self.dest0, self.dest1 = controller.car_plan.destination_nodes[1], controller.car_plan.destination_nodes[0]

        # self.start1, self.dest1 = controller.car_plan.start_nodes[1], controller.car_plan.destination_nodes[1]

        start0, dest0 = self.start0, self.dest0 
        start1, dest1 = self.start1, self.dest1  
        ...
        # if has_origin and has_swap:
            print(1)
            # print("pair1: ", len(path1) + len(path2))
            # print("pair2: ", len(path3) + len(path4))
            # print(start0, dest0, start1, dest1)
            # print(start0, dest1, start1, dest0)
            return (start0, dest0, start1, dest1) if len(path1) + len(path2) <= len(path3) + len(path4) else (start1, dest1, start0, dest0)
            # if len(path1) + len(path2) <= len(path3) + len(path4):  
        elif (path1 and path2) and (path3 or path4): 
            print(2)
            return (start0, dest0, start1, dest1)
            # return (start0, dest1, start1, dest0) 
        elif (path1 or path2) and (path3 and path4): 
            print(3)
        ...
        if path1 and not path2: 
            print(5)
            return (start0, dest0, start1, dest1)
        if not path3 and path4: 
            print(6)
            return (start0, dest1, start1, dest0)
        if path3 and not path4: 
            print(7)
    ...
    async def run(self):  
        ...
        self.controllerA.load = 13
        self.controllerA.unload = self.controllerA.car_plan.unloads[0]
        # path_to_load = self.controllerA.car_plan.dijkstra(6, 11)
        # print(path_to_load)
        self.controllerB.fetch_map() 
        self.controllerB.load = 5
        self.controllerB.unload = self.controllerB.car_plan.unloads[1]

        print("Load A: ",self.controllerA.load)
        print("Unload A: ",self.controllerA.unload)
        print("Load B: ",self.controllerB.load)
        print("Unload B: ",self.controllerB.unload)
        # exit()
        if priority == 'A':
            print("A")
            ...
            # self.controllerA.get_path_direction([],self.controllerA.original_start,
            ...
            self.controllerA.get_path_direction([],self.controllerA.original_start,
                                                self.controllerA.load)
            self.controllerB.get_path_direction(self.controllerA.path,
                                                self.controllerB.original_start,
                                                self.controllerB.load) 
            
        elif priority == 'B': 
            print("B")
            self.controllerB.original_start = start0
            self.controllerB.original_destination = dest0
            self.controllerA.original_start = start1
            self.controllerA.original_destination = dest1
            # self.controllerB.get_path_direction([],start0,dest0)  
            # self.controllerA.get_path_direction(self.controllerB.path,start1,dest1)

            # self.controllerB.get_path_direction([],
            #                                     self.controllerB.original_start,
            #                                     self.controllerB.original_destination) 
...
class CarController: 
    ...
    async def proccess(self, payload_str): 
        ...
        if status_type == 'checkpoint' and not self.check_checkpoint(self.path,value):
            self.count_waiting += 1 
            self.current_position = value 
            if self.count_waiting >= 3: 
                print("[RECOVERY] Car reached a wrong checkpoint. Initiating 45° turn")
                await self.send_command(self.pub_topic, "TURN 45 DEGREE")
                await asyncio.sleep(2)
                await self.send_command(self.pub_topic, "MOVE OUT")                
                self.path = []
